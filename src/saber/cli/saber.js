...
console.log("authority =",Pauthority);
  ...
  let infoBefore;
  let ret = [];
  await mintA.mintTo(userAccountA, wallet, [], 7000000000);
  infoBefore = await mintA.getAccountInfo(tokenAccountA);
  console.log("amount tokenAccountA : " + infoBefore.amount.toNumber())
  await mintA.approve(userAccountA, authority, wallet, [], 1000,connection);
  ...
  console.log("****************************************")
   await mintB.mintTo(userAccountB, wallet, [], 7000000000);
   await mintB.approve(userAccountB, authority, wallet, [], 1000,connection);
...
export async function createPoolMint(wallet, connection) {
 ...
 let instruction1= await mintA.mintToReturnInstruction(tokenAccountA,wallet,[],10000000000);
 let instruction2= await mintB.mintToReturnInstruction(tokenAccountB,wallet,[],10000000000);
 const transaction=new Transaction();
